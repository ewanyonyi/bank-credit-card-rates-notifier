...
class App extends Component {
  ...
  componentDidMount() {
    axios
      .get(
        `https://api-dev.capitalone.co.uk/open-banking/v1/consumer-credit-cards`
      )
      .then(res => {
        const aprRate = res.data.data[0].Details.ConsumerCreditCardItem.APRRate;
        const legalName =
          res.data.data[0].Organisation.ParentOrganisation.OrganisationName
            .LegalName;
    ...
    setInterval(() => {
      axios
        ...
        .then(res => {
          const aprRate =
            res.data.data[0].Details.ConsumerCreditCardItem.APRRate;
          const legalName =
            res.data.data[0].Organisation.ParentOrganisation.OrganisationName
              .LegalName;
          const trademarkID = res.data.data[0].Organisation.Brand.TrademarkID;
